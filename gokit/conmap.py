...
class conmaps():
    ...
    #Writing xyz file to : native_ca.pdb.xyz
        ...
        def check_glycineh(self,pdbfile,skiph):
            ...
            #check if glycine has a hydrogen atom.
                        ...
                        if CB1 and CB2:
                            ...
                            if (distance*10<cutoff).any():
                                if np.amin(distance)*10<scaling*cb_dist*10:
                                    contacts.append([i[0],i[1],np.amin(distance)*10,cb_dist*10])
                                    #print (i[0],i[1],np.amin(distance)*10)
            contacts = np.asarray(contacts)
            np.savetxt('sidechain.t', contacts)
            return contacts

        def get_bb_sc_contacts(self,pdbfile,cutoff,scaling,separation):
            print(">> in bb_sc_contacts", pdbfile, "cutoff =", cutoff)
...
def main():
       ...
       parser.add_argument("--cutoff", help="Cut-off for calculating contact map.")
       parser.add_argument("--scaling", help="Scaling for calculating contact map .")
       parser.add_argument('--get_seq', help='Get sequence from pdb')
       parser.add_argument("--native", help="Give CA-CA pdbfile as native file")
       parser.add_argument("--aa_pdb", help="Give all-atom pdbfile")
       parser.add_argument("--traj", help="Give trajectory filename. Else traj.xtc assumed")
       parser.add_argument('--traj_list', help='List of trajectories. Comma separated.No spaces.', type=str)
       parser.add_argument("--frames", help="Give three comma separated integers start, stop, step",type=str)
       parser.add_argument("--xyz_out", help="Give XYZ output file name for extracting trajectory.")
       parser.add_argument("--xyz_traj", help="Give XYZ trajectory file containing lots of xyz structures.")
       parser.add_argument("--get_polar", help="Get positions of polar residues ")
       parser.add_argument("--get_charged", help="Get positions of charged residues ")
       parser.add_argument("--get_hp", help="Get positions of hydrophobic residues ")
       parser.add_argument("--ext_pair", help="Get contact pairs from any other program(say SMOG). Give smog.contacts(reads columns 2 and 4)")
       parser.add_argument("--ext_con_map", help="External contact map provided (0 or 1)")
       parser.add_argument("--con_prob", action='store_true', default=False,help="Get contact probabilities from xyz or xtc trajectory. ")
       parser.add_argument("--pl_fe", help="Plot \" Free energy profile\". Input file containing only Q values. e.g. 1.map ")
       parser.add_argument("--cluster", action='store_true', default=False,help="Hierarchical clustering of structures in xtc file.")
       parser.add_argument('--get_energies', action='store_true', default=False,help="Get energies from single point OPTIM runs for each frame in xyz trajectory ")
       parser.add_argument('--check_pdb',help="Give pdb  ID. Outputs smog readable pdb ")
       parser.add_argument('--get_pdb', help="Give pdb ID. Downloads pdb ")
       parser.add_argument('--gconmap', help="Give pdb file. Generate contact map from all atom pdb ")
       parser.add_argument("--pl_map", action='store_true', default=False,
                           help="Plot contact map from gconmap.")
       parser.add_argument('--renumber_res', help="Give pdb file. Renumbers residues starting from 1 ")
       parser.add_argument('--hphobic', help='Generate hydrophobic contacts. Give PDB file. Also need native_pairs file for eliminating duplicates.')
       parser.add_argument("--bb_con",
                           help="Calculate only backbone-backbone contacts. ")
       ...
       parser.add_argument("--rge", action='store_true', default=False,
                           help="Get E vs rog plot for entire trajectory.")
       parser.add_argument("--count_trans",
       ...
       parser.add_argument("--cif2pdb", help="ciffile pdbfile")
       parser.add_argument("--pdb2xyz", help="pdbfile xyzfile")
       parser.add_argument("--xyz2pdb", help="xyzfile pdbfile;add --native native_ca.pdb")
       parser.add_argument("--dssp", help="xyzfile pdbfile;add --native native_ca.pdb")
...
#for testing only
       ...
       if args.get_pdb:
           pdbid=args.get_pdb
           pdb1=PDBList()
           pdb1.retrieve_pdb_file(pdbid, pdir='./',overwrite=True)
