...
class esbm(object):
    ...
    # For help type python eSBM.py --help
        ...
        def globals(self,Ka1,Kb1,Kd1,CA_rad1,skip_glycine1,sopc1,dswap1,btparams1,CAcom1,hphobic1,hpstrength1,hpdist1,dsb1):
            ...
            global skip_glycine,sopc,dswap,btparams,CAcom,hphobic,dsb
            skip_glycine=skip_glycine1;dswap=dswap1;btparams=btparams1;sopc=sopc1;CAcom=CAcom1;hphobic=hphobic1;dsb=dsb1
        ...
        # def MJparams(self,filename):
        #     print ('Reading',filename,'for MJ params')
        #     #convert MJ table of epsilons to dictionary
        #     f1 = open('mjmap.dat', "w+")
        #     lines = open(filename).read().split("\n")
        #     #matrix=np.loadtxt(filename,dtype=float)
        #
        #     resnames=str.split('C  M   F   I   L   V   W   Y   A   G   T   S   N   Q   D   E   H   R   K   P')
        #     for i in range(0,20):
        #         line=lines[i].split()
        #         for j in range(0,i+1):
        #             print (resnames[i]+resnames[j],line[j])
        #             #line=lines[i].split()
        #             #print (line)
        #
        #         #a=resnames[i]+resnames[j]
        #
        #             f1.write('%s %f\n' % (resnames[i]+resnames[j],float(line[j])))
        #     f1.close()
        #
        #
        #     return
        ...
        def write_CB_to_native(self,pdbfile,sopc):
            ...
            #print ("skip_glycine,sopc",skip_glycine,sopc)
...
#                    k[0], k[1], k[2], k[3], k[4], k[5], k[6], k[7], k[8], k[9], k[10], k[11]))
        ...
        def get_CB_radius(self,atomtypes,nativefile,sopc,scaling):
            ...
            if sopc:
                ...
                radius=(0.8*(radius+3.8))/2 #from greddy JPCB Supplementary.(https://pubs.acs.org/doi/suppl/10.1021/acs.jpcb.6b13100/suppl_file/jp6b13100_si_001.pdf)
                return radius
            assert atomtypes==2
            ...
            radius=Y.get_distances(pdbfile,pairs)*10
        ...
        def write_atomtypes_section(self, pdbfile, atomtype,sopc,CA_rad):
            CA_rad=float(CA_rad)
            print ('>> in atomtypes_section',pdbfile,atomtype,sopc,CA_rad)
            assert atomtype<=2
            Y=conmaps()
            f = open('SBM.INP', "a")
            seq = Y.get_sequence(pdbfile)
            ...
            if atomtype==2:
                CB_rad = self.get_CB_radius(atomtype,'native_cb.pdb',False,1)
        ...
        def write_contacts_section(self,pdbfile,nativefile,btfile,cutoff,atomtype,sopc,btparams,dswap):
            ...
            if atomtype==1:
                ...
                nc = Y.all_atom_contacts(pdbfile, cutoff)
                dist_ca = nc[:, 3];
                #print (dist_ca)
                contacts_ca = nc[:, (0, 1)]
                ...
                if hphobic:
                    ...
                    print ("Adding hydrophobic contacts: Distance =",hpdist)
                    ...
                    for i in p:
                        ...
                        f.write('%s\t%s\t%d\t%f\t%e\n' % (
                        dict1[int(i[0])] + 1, dict1[int(i[1])] + 1, contacttype, hpdist * 10, hpstrength))
                        f2.write(' %s\t%d\t%s\t%d\n' % ('1', dict1[int(i[0])] + 1, '1', dict1[int(i[1])] + 1))
                        counthp = counthp + 1
                ...
                for i in xrange(0, len(sc)):
                    ...
                    #btparams=True
                    if btparams:
                        #print ("BTparameters for side-chain side-chain interactions. btparams=",btparams)
                       # boltzmann_kcal_mol=float(0.0019872041)
                        ...
                        print (res1bt,res2bt,strength_CB)
                ...
                for i in xrange(0,len(nc_sc)):
                    ...
                    pair = np.reshape(pair, (1, 2))
                    dist = md.compute_distances(traj, pair, periodic=False, opt=True)[0][0] * 10
                    #print dist,nc_sc[i][3],nc_sc[i][2],res1+!,res2
                    contacts.append([res1 + 1, res2 + 1, int(contacttype), dist, strength_CA])
                    #f4.write(' %s\t%d\t%s\t%d\n' % ('1', int(res1) + 1, '1', int(res2) + 1))
...
########GROMACS topology section. Make separate class later.##########
        ...
        def write_gro_atomtypes(self,topfilename,atomtypes,pdbfile,sopc,CA_rad):
            print (">> writing GROMACS atomtypes section",topfilename,atomtypes,pdbfile,sopc,CA_rad)
            #CA_rad=float(3.8)
            f = open(topfilename, "a")
            assert atomtypes<=2
            a=self.write_atomtypes_section(pdbfile,atomtypes,sopc,CA_rad)
            assert a[0][0]=='CA'
            b=a[0]
            CA_rad=(CA_rad/10)**12
            f.write('%s\n'%('[ atomtypes ]'))
        ...
        def write_gromacs_top(self,topfilename,atomtypes,pdbfile,nativefile,CA_rad,sopc,btparams,Ka,Kb,Kd,cutoff):
            #Order for SBM file
            assert atomtypes <= 2
            #2 = 10-12 potential
            print (atomtypes)
            ...
            self.write_gro_atomtypes(topfilename,atomtypes,pdbfile,sopc,CA_rad)
...
def main():
    ...
    parser.add_argument("--CA_rad", help="Radius for C-alpha atom. Default=4.0")
    parser.add_argument("--CAcom", help="Place C-alpha at COM of backbone")
    parser.add_argument("--Cb_rad", help="Statistically derived values by default for attype 2.")
    parser.add_argument("--Kb", help="Kbond")
    parser.add_argument("--Ka", help="Kangle")
    parser.add_argument("--Kd", help="Kdihedral")
    ...
    parser.add_argument("--attype", help="Number of atom types. E.g. 1 for CA, 2 for CA and CB")
    parser.add_argument("--interaction", action='store_true', default=False, help='User defined interactions in file interaction.dat.')
    parser.add_argument("--CBcom",  action='store_true', default=False,help='Put CB at center of mass of side-chain (no hydrogens)')
    parser.add_argument("--dsb", action='store_true', help="Use desolvation barrier potential for contacts.")
    parser.add_argument("--native_ca", help='Native file with only C-alphas. Just grep pdb. ')
    ...
    parser.add_argument("--pdbgro", help='Name for .gro file.')
    parser.add_argument("--w_native", help='Write native files, CA and CA-CB from all atom PDB file.')
    parser.add_argument("--pl_map", action='store_true', default=False, help='Plot contact map for two bead model')
    parser.add_argument("--skip_glycine", action='store_true', default=False, help='Skip putting Cbeta on glycine')
    parser.add_argument("--dswap", action='store_true', default=False, help='For domain swapping runs. Symmetrised SBM is generated.')
    parser.add_argument('--hphobic',action='store_true',help='Generate hydrophobic contacts.')
    parser.add_argument('--hpdist', help='Equilibrium distance for hydrophobic contacts.')
    parser.add_argument('--hpstrength',help='Strength with which hydrophobic contacts interact.')
    parser.add_argument('--ext_conmap', help='External contact map in format chain res chain res')
    ...
    X.globals(Ka,Kb,Kd,CA_rad,skip_glycine,sopc,dswap,btparams,CAcom,hphobic,hpstrength,hpdist,dsb)
    ...
    if args.attype and args.aa_pdb:
        ...
        if int(args.attype)==2:
            if args.CBcom:
                nativefile='native_cb.pdb'
                X.write_gromacs_top(topfile,int(atomtypes),pdbfile,nativefile,CA_rad,sopc,btparams,Ka,Kb,Kd,cutoff)
            else:
