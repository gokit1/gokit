...
from Lab16_toolkit import PDB16_kit
#Some conventions:
...
class esbm(Select,object):
    ...
    # For help type python eSBM.py --help
        ...
        def write_CB_to_native(self,pdbfile,sopc):
            ...
            for i in line:
                ...
                if i[12:16].strip()=="CA":
                    ...
                    k[3] = i[17:20].rstrip().lstrip().ljust(3)  # resname#1s
                    #print k[3]
                    k[4] = i[21].rjust(1)  # Astring
                    k[5] = i[22:26].rstrip().lstrip().rjust(4)  # resnum
                    if CAcom:
                    ...
                    #if not GLYCINE
                    #print (CB_dict.keys())
                    if int(i[22:26].rstrip().lstrip()) in CB_dict.keys():
                        #print (int(j[5]))
                        count=count+1
        ...
        def get_farthest_atom_in_residue(self,pdbfile,index):
            ...
            #returning if no sidechain residue (as in GLY)
            if len(atomlist) == 0:
                #putting unrealistic co-ordinate
        ...
        def get_farthest_atom_in_residue_list(self,pdbfile,index_list):
            ...
            for index in index_list:
                ...
                if len(atomlist) == 0:
                    #sending 
                    #putting unrealistic coo-rodinate
                    farrthest_atom.append(np.array([-10**10,-10**10,-10**10]))
                    #np.argmax doens't work with emplty array
                    continue
                    #escapnig current iteration
                pairs=[]
                for i in atomlist:
                    pairs+=[([i,ca_atom[0]])]
                pairs=np.asarray(pairs,dtype=int)
                ...
                dist=md.compute_distances(traj,pairs,periodic=False, opt=True)[0]
                farthest=pairs[np.argmax(dist)][0]
                farrthest_atom.append(traj.xyz[0][farthest])
            return farrthest_atom
        def get_farthest_CB_coords(self,pdbfile):
            print(">> Finding farthest sidechain atom co-ordinates")
            CB=[]
            Y=conmaps()
            ...
            #    cb_far = self.get_farthest_atom_in_residue(pdbfile,i)
            #    if (cb_far==np.array([-10**10,-10**10,-10**10])).all(): 
            #        print ("Skipping GLY residue",resi_num[i])
            #        continue
            #    else:                    
            #        #md_trak returns values in nm, converting to A by myltiplying CB_far by 10
            #        CB.append([cb_far*10,resi_num[i]]) 
            
            CB_far_list = self.get_farthest_atom_in_residue_list(pdbfile,range(n_res))
            CB_list=[]
            for i in range(len(CB_far_list)):
                if (CB_far_list[i]==np.array([-10**10,-10**10,-10**10])).all():
                    print ("Skipping GLY residue",resi_num[i])
                    continue
                else:
                    CB_list.append([ CB_far_list[i]*10,resi_num[i]])
            
            #assert(len(CB)==len(CB_list))
            #count = 0
            #for i in range(0,len(CB_list)):
            #    if (CB[i][0]==CB_list[i][0]).all() and CB[i][1]==CB_list[i][1]:
            #        count = count + 1
            #        print (CB[i],CB_list[i])
                    
            #COM1 = np.array(COM1).reshape(len(COM1) / 2, 2)
            #CB=np.array(CB).reshape(len(CB),2)
            CB_list=np.array(CB_list).reshape(len(CB_list),2)
            #print (CB_list)
            return (CB_list)
        def get_CB_radius(self,atomtypes,nativefile,sopc,scaling,CBradii):
            ...
            radius=np.float_(radius).reshape(1,len(ncb))
        ...
        def get_atom_names(self,pdbfile,atomtype,sopc):
            ...
            if atomtype==1:
                ...
                test_count = 0
                for i in atomname:
                    #print ("Testing-error",i,test_count)
        ...
        def write_atomtypes_section(self, pdbfile, atomtype,sopc,CA_rad,CBcom,CBradii):
            ...
            if atomtype==2:
                ...
                print ("Number of glycines = ",count_gly)
                #opening native_cb.pdb for extracting residue numbers
                #native_cb = PDBParser().get_structure("test","native_cb.pdb")
                #all = native_cb.get_residues()
                #residues = list()
                #for i in all:
                #    residues.append(i)
                #for i in residues:
                ...
                for i in seq:
                    ...
                    if i != glyname:
                        ...
                        atomname.append([x,atommass[count+1],atomcharge[count+1],atomptype[count],c6,CB_rad[0][count_cb],d[i]])
        ...
        def write_atoms_section(self,pdbfile,atomtype,sopc):
            ...
            ncb=len(seq) - len(gly)
            glyname='G'
            print (skip_glycine,"Skip glycine")
            if not skip_glycine:
                ncb=len(seq)
                glyname='G111'
            ...
            if atomtype==1:
                ...
                if sopc:
                   ...
                   nn_angles = Y.get_SOPC_non_native_angles(pdbfile, nativefile, 3.8, 1.0)
                    ...
                    L = PDB16_kit()#__TESTING__
                    #separation is always >=
                    cacasep=4;cacbsep=3;cbcbsep=3
                    print ("using PDB16_kit")#__TESTING__
        ...
        def get_chain_termianl_residue_index(self,nativefile):
            ...
            for r in residues:
                ...
                terminal_residue_index.append(len([x for x in r])-1)
        ...
        def write_bonds_section(self,nativefile,Kb,atomtype):
            ...
            #getting chain terminal indices
            ...
            for residue_index in xrange(len(CA_indices)-1):
                assert (Y.get_atom_name(top, CA_indices[residue_index])[0].strip() == 'CA')
            ...
            for i in all:
                ...
                f.write(' %d\t%d\t%d\t%8.5f\t%e\n' % (i[0]+1, i[1]+1, bondtype, i[2], K_bond))
                    #print (dsb_pairs)
            return all1
        def write_angles_section(self,nativefile,Ka,atomtype):
            ...
            if atomtype == 1:
                ...
                #loading native pdb
                fcheck = open(nativefile); native_pdb = fcheck.readlines()
                fcheck.close()

                #CA and CB indices in native
                CA_in_native = [int(x[6:11])-1 for x in native_pdb if x[12:16].strip()=="CA"]
                CB_in_native = [int(x[6:11])-1 for x in native_pdb if x[12:16].strip()=="CB"]

                #get terminal residues indices
                terminal_residue = self.get_chain_termianl_residue_index(nativefile)
                termianl_CA = list()
                
                #Adding CAi-1_CAi_CAi+1 triplets
                for i in xrange(1,len(CA_in_native)-1):
                ...
                for b in CB_in_native:
                    ...
                    if a not in termianl_CA and b!=CB_in_native[len(CB_in_native)-1]:                    
                        #b:a:a+1    #if a is in termial CA, donot include the triplet
            ...
            f = open('SBM.INP', "a")
            f.write('\n%d\t%s\n' % (num_angles, 'angles'))
            #CA_angles include CB also in case of atomtype = 2
        ...
        def get_CB_chiral_dihedrals(self,nativefile,terminal_residue):
            ...
            #    print (i)
            return quadraplets 
        def write_dihedrals_section(self,nativefile,Kd,atomtypes):
            #Only over C-alpha atoms for Cheung-Thirumalai model.
            #for CB-CB model, an improper dihedral term is added to avoid
            #flipping of CB (to maintain CA as a Chiral center)
...
########GROMACS topology section. Make separate class later.##########
        ...
        def write_gro_angles(self,topfilename,atomtypes,nativefile,Ka):
            ...
            angles=self.write_angles_section(nativefile,Ka,atomtypes)
        ...
        def write_gro_dihedrals(self,topfilename,atomtypes,nativefile,Kd):
            ...
            f = open(topfilename, "a")
            f.write('\n%s\n' % ('[ dihedrals ]'))
            d=self.write_dihedrals_section(nativefile,Kd,atomtypes)
        ...
        def write_gro_pairs(self,topfilename,atomtypes,nativefile,pdbfile,contacttype,cutoff,sopc,btparams):
            ...
            #charge contacts if CBcvharge is true
            #if CBcharge and atomtypes == 2:
            #    q_contacts,q_dist = self.chargeContacts(pdbfile,nativefile,cutoff)
            #    for q in range(0,len(q_contacts)):
            #        f.write('\t%d\t%d\t%s\t%e\t%e\n' % (i[0],i[1],'1',A,B))
...
def main():
   ...
   # X.get_farthest_atom_in_residue('1qys_H.pdb',91)
    ...
    if args.CBcharge:
        CBcharge = True
    X.globals(Ka,Kb,Kd,CA_rad,skip_glycine,sopc,dswap,btparams,CAcom,hphobic,hpstrength,hpdist,dsb,mjmap,btmap,CBfar,CBcharge,CB_rad)
