...
echo "Using 2C62.pdb as the test input."
echo "This is a homodimeric structure, we will be using all chains"
echo "Help output have been generated in USAGE.txt"
python ../../nucproSBM.py --help >> USAGE.txt
echo "======================================================="
echo "Case I: 2C62.pdb as input pdb. Using charge on CB (centre at the co-ordinate of farthest atom) with Monovalent ionic conc to be 0.01M. CA and CB radius set to 1.9 and 1.5A respectively. For DNA beads, using default radius (refer to USAGE.txt). Using Debye-Huckel electrostatic potential"
echo "Command used: $ python ../../nucproSBM.py --aa_pd 2C62.pdb --grotop gromacs.top -pdbgro gromacs.gro  --CA_rad 1.9 --skip_glycine  --CBfar --CBcharge --dielec 70 --iconc 0.01 --CB_rad 1.5 --debye --all_chain"

echo "Ready to proceed [yes=1/no=0]? (0)"
read input
if [ $input -eq 1 ]
then
python ../../nucproSBM.py --aa_pd 2C62.pdb --grotop gromacs.top -pdbgro gromacs.gro  --CA_rad 1.9 --skip_glycine  --CBfar --CBcharge --dielec 70 --iconc 0.01 --CB_rad 1.5 --debye --all_chain 

mkdir Output/CaseI
rm -r Output/CaseI/*
mv * Output/CaseI/
cp Output/CaseI/2C62.pdb .
cp Output/CaseI/run.sh .
mv Output/CaseI/polyT.pdb .
mv Output/CaseI/2C62_DNA.pdb .
fi

echo "======================================================="

echo "Case II: CB at Sidechain COM"
echo "Command used: $ python ../../nucproSBM.py --aa_pd 2C62.pdb --grotop gromacs.top -pdbgro gromacs.gro  --skip_glycine  --CBcharge --dielec 70 --iconc 0.01 --debye --all_chain"
echo "Ready to proceed [yes=1/no=0]?(0)"
read input
if [ $input -eq 1 ]
then
python ../../nucproSBM.py --aa_pd 2C62.pdb --grotop gromacs.top -pdbgro gromacs.gro   --skip_glycine  --CBcharge --dielec 70 --iconc 0.01 --debye --all_chain
mkdir Output/CaseII
rm -r Output/CaseII/*
mv * Output/CaseII/
cp Output/CaseII/2C62.pdb .
cp Output/CaseII/run.sh .
mv Output/CaseII/polyT.pdb .
mv Output/CaseII/2C62_DNA.pdb .
fi
python ../../nucproSBM.py --help >> USAGE.txt

echo "======================================================="
echo "Case III: 2C62.pdb as input pdb. Using different parameters for Protein-DNA interface."
...
rm -r Output/CaseIII/*
mv * Output/CaseIII/
cp Output/CaseIII/2C62.pdb .
cp Output/CaseIII/run.sh .
mv Output/CaseIII/polyT.pdb .
mv Output/CaseIII/2C62_DNA.pdb .
fi

echo "======================================================="
echo "Case IV: 2C62.pdb as input pdb. Using different DNA structure along with different parameters for Protein-DNA interface."
...
echo "The DNA sequence used is PolyT of lenght 16 bases (same as the native bound DNA strand)"
...
rm -r Output/CaseIV/*
mv * Output/CaseIV/
cp Output/CaseIV/2C62.pdb .
cp Output/CaseIV/polyT.pdb .
cp Output/CaseIV/run.sh .
mv Output/CaseIV/polyT.pdb .
mv Output/CaseIV/2C62_DNA.pdb .
fi

echo "======================================================="
echo "Case V: 2C62.pdb as input pdb. Using native DNA sequecne structure as custom input (no notive contacts) along with different parameters for Protein-DNA interface."
echo "In the case an interface file is used: nucpro_interface.input. The file is used for providing custom repulsion term when accounting for DNA-Protein interaction"
...
rm -r Output/CaseV/*
mv * Output/CaseV/
cp Output/CaseV/2C62.pdb .
cp Output/CaseV/2C62_DNA.pdb .
cp Output/CaseV/run.sh .
mv Output/CaseV/polyT.pdb .
mv Output/CaseV/2C62_DNA.pdb .
fi
